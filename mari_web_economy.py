...
class Economy(PaidPasses):
 def __init__(self,b,error):
  ...
  CREATE TABLE IF NOT EXISTS mari_web_volatility_states(symbol TEXT,start TEXT,expires TEXT NOT NULL,state INTEGER NOT NULL CHECK(state BETWEEN 0 AND 4),PRIMARY KEY(symbol,start));
 ...
 def stock_slot(self):
  ...
  return now.replace(minute=now.minute//PRICE_INTERVAL_MINUTES*PRICE_INTERVAL_MINUTES,second=0,microsecond=0)
 ...
 def delist(self,symbol,slot):
  ...
  for uid,side,lev,qty,cost,notional in rows:
   total=max(0,cost+(floor*qty-notional)*(1 if side=='long' else -1));rid='delist:'+secrets.token_hex(16)
   self.db.execute('INSERT INTO balances VALUES(?,?) ON CONFLICT(user_id) DO UPDATE SET balance=balance+excluded.balance',(uid,total))
   self.db.execute('INSERT INTO mari_web_stock_trades VALUES(?,?,?,?,?,?,?,?,?)',(rid,uid,symbol,side+'_close',qty,floor,total,total-cost,datetime.fromisoformat(slot).timestamp()))
  ...
  self.db.execute('INSERT OR IGNORE INTO mari_web_stock_days VALUES(?,?,?,?)',(symbol,slot,floor,floor))
  ...
  self.db.execute('INSERT OR REPLACE INTO mari_web_stock_news VALUES(?,?,?,?)',(symbol,slot,f'[{name}] 최저가 도달…상장폐지', f'시작가 {self.listing_price(symbol):,}마리 대비 99% 하락해 거래가 종료됐습니다. 남은 롱·숏 포지션은 {floor:,}마리 기준으로 정산되었습니다.'))
 ...
 def settle(self):
  ...
  with self.db:
   ...
   volatility_start=datetime.fromisoformat(self.db.execute("SELECT value FROM mari_web_stock_settings WHERE key='volatility_start_v1'").fetchone()[0])
   ...
   self.db.execute("INSERT OR IGNORE INTO mari_web_stock_settings VALUES('log_market_start',?)",((slot+timedelta(minutes=PRICE_INTERVAL_MINUTES)).isoformat(),))
   ...
   for symbol,_,_ in STOCKS:
    ...
    if not row:
     ...
     self.db.execute('INSERT INTO mari_web_stock_days VALUES(?,?,?,?)',(symbol,slot.isoformat(),initial,initial));continue
    price,day=row
    if price<=floor:
     self.delist(symbol,slot.isoformat());self.db.execute('UPDATE mari_web_stocks SET price=?,day=? WHERE symbol=?',(floor,slot.isoformat(),symbol));continue
 ...
 def market(self,member):
  ...
  return {'positionVersion':4,'bulkClose':True,'positionClose':True,'positions':self.positions(member.id,items),'news':news,'stocks':items,'balance':self.balance(member.id),'day':self.today().isoformat(),'nextUpdate':(self.stock_slot()+timedelta(minutes=PRICE_INTERVAL_MINUTES)).isoformat(),'trades':trades}
