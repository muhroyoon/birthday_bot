...
def step(s, x, y, tap):
 ...
 if g=='runner':
  ...
  for o in s['objects']:
   old=o['x'];o['x']-=speed
   if not o.get('hit') and old+o['w']>76 and o['x']<100 and s['y']+34>334-o['h'] and t>s.get('immune',0):
    s['lives']-=1;o['hit']=True;s['immune']=t+12;s['message']='충돌!'
  s['objects']=[o for o in s['objects'] if o['x']>-60];s['score']=t
 elif g=='dodge':
  ...
  if s['phase']=='wait' and t>=s['until']:s['phase']='bite';s['until']=t+20;s['message']='입질! 지금 낚아채세요'
   ...
   if tap:s.update(phase='fight',progress=0,tension=50,message='감기와 놓기로 장력을 조절하세요')
   elif t>=s['until']:s['phase']='wait';s['until']=t+20+rand(s,25);s['message']='놓쳤어요. 다음 입질을 기다려요'
  elif s['phase']=='fight':
   s['tension']+=5 if x==1 else -3
   s['progress']=max(0,s['progress']+(3 if 25<=s['tension']<=80 else -1))
   if s['tension']<=0 or s['tension']>=100:
    s['phase']='wait';s['until']=t+20+rand(s,25);s['message']='줄이 풀렸어요' if s['tension']<=0 else '줄이 끊어졌어요'
   elif s['progress']>=100:
  ...
  if s.get('rules',1)>=3 and t>=450 and len(enemies)<3:
   ...
   if free:
    v=free[rand(s,len(free))];enemies.append({'x':v%20,'y':v//20,'dx':-1,'dy':1})
  if x or y:
   ...
   if (nx,ny)!=(s['x'],s['y']):
    if cell in trail:s['lives']-=1;s['x']=0;s['y']=0;trail.clear()
     ...
     if cell not in claimed:trail.append(cell)
      ...
      walls=claimed|set(trail);outside=set();todo=[enemy['y']*20+enemy['x'] for enemy in enemies]
      while todo:
       v=todo.pop()
  ...
  for e in (enemies if t%2==0 else []):
   ...
   for _ in range(2 if s.get('rules',1)>=3 and t>=600 else 1):
    for axis,delta in [('x','dx'),('y','dy')]:
     ...
     if e['y']*20+e['x'] in trail:hit=True
  if hit:s['lives']-=1;trail.clear();s['x']=0;s['y']=0;s['message']='선이 끊겼어요'
...
class Adventure:
 ...
 def public(self,row):
  ...
  s=json.loads(row[4])
  if s['game'] not in ('territory','dodge','tower','runner','fishing'):s.pop('rng',None)
  else:s['motionVersion']=3
  if s['game']=='memory':
   seq=s.pop('sequence');
   if s.get('rules',1)>=3:s['showSequence']=seq if s['phase']=='show' else [];s['sequenceLength']=len(seq)
 ...
 def control(self,member,data):
  ...
  if row[3]=='memory' and s.get('rules',1)>=3:
   if data.get('memoryReady') is True and s['phase']=='show':
