...
class Weekly:
    def __init__(self,b):
        ...
        CREATE TABLE IF NOT EXISTS mari_web_weekly_policy(id TEXT PRIMARY KEY, starts_week INTEGER NOT NULL);
    ...
    def canonical_board(self,board,week):
        ...
        return board.rsplit(':',1)[0]+':mouse' if self.unified(board,week) else board
    ...
    def rows(self,week,board):
        aliases=[board]
        if self.unified(board,week):aliases=[board.rsplit(':',1)[0]+':'+device for device in ('mouse','touch')]
        records=self.db.execute('SELECT user_id,guild_id,at,score,accuracy,average_ms,metrics FROM mari_web_weekly_records WHERE week=? AND board IN ('+','.join('?' for _ in aliases)+') ORDER BY at,id',(week,*aliases)).fetchall()
    ...
    def settle(self,now=None):
        ...
        with self.db:
            while deadline<=now:
                week=deadline-WEEK
                boards=sorted({self.canonical_board(r[0],week) for r in self.db.execute('SELECT DISTINCT board FROM mari_web_weekly_records WHERE week=?',(week,))})
                for board in boards:
